...
public class Verifier {
    ...
    public boolean verifyNumberAndDelimiterDomain(String input, HashSet<String> delimiters) {
        ...
        for (int currentIndex = 0; currentIndex < input.length(); currentIndex++) {
            ...
            // 구분자는 현재 위치부터 다음 숫자 전 위치까지
            int nextDigitIndex = findNextDigitIndex(input, currentIndex);
            String currentDelimiter = input.substring(currentIndex, nextDigitIndex);
    ...
    private int findNextDigitIndex(String input, int startIndex) {
